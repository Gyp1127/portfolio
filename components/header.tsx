        ...
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-stone-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
            {
                links.map(link => (
                    ...
                    > 
                        <Link className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-stone-950 transition", {"text-stone-950": activeSection === link.name})} 
                            href={link.hash}
                            onClick={() => {
...

                            {
                                ...
                                <motion.span className="bg-stone-100 rounded-full absolute inset-0 -z-10"
